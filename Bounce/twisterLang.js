...
app.get('/', (req, res) => {
  ...
  <body>
      ...
      <script>
          ...
          setTimeout(function() {
              window.location.href = 'stormbaan.html'; // Zorg ervoor dat deze pagina bestaat
          }, 5000);
      </script>
  </body>
  </html>
...
function displayData(timeframe) {
    ...
    switch (timeframe) {
        case 'month':
            ...
            sql = `SELECT * FROM \`tijden\` WHERE Datum BETWEEN '${startOfMonth}' AND '${endOfMonth}' AND Soort = 'twister' ORDER BY Tijd ASC LIMIT 5`;
            break;
        case 'year':
            ...
            sql = `SELECT * FROM \`tijden\` WHERE Datum BETWEEN '${startOfYear}' AND '${endOfYear}' AND Soort = 'twister' ORDER BY Tijd ASC LIMIT 5`;
            break;
        default:
